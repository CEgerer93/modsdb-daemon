...
# Dir=baseDir+"/"+beta+"/"+ensem+"/"+momInStr+"/pdf_m"+lm+"_db/TRUTHRUNS_MELTEMI/"+str(src)

# Set the output name for the sdb
sdbname="/data/cpegerer/SDB_pion/"+beta+"/"+ensem+"/"+momOutStr+"/"+str(src)
# sdbname="/data/cpegerer/SDB_pion/"+beta+"/"+ensem+"/"+momOutStr+"/TRUTHRUNS_MELTEMI/"+str(src)
...
def elemblock4pt(cfg,ccm,g):
    ...
    # Set the location and naming of resulting sdb file
    xml.openTag("sdb")
    xml.writeTag("name",sdbname+"/h00z/pion_4pt.sdb"+str(cfg))
    xml.closeTag()
