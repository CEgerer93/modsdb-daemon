...
parser.add_option("-a", "--maxP", type="int", default=0,
                  help='Set the maximum z-momentum (default 0)')
...
def elemblock(c,f,g,x,y,z,p):

    # snkLor=[c, options.g1, options.g2, x, y, z]
    snkLor=[c, f, g, x, y, z]
    mom_f=[0,0,p]
    mom_i=[0,0,p]
...
for c in range(0,4): # loop over the cc_mass
    # Loop over gamma matrix insertions ... for Raza
    for f in [1, 2, 4]: # gamma1 loop
        # for p in range(1,options.maxP+1):
            for x in range(1,options.xsep+1):
                for y in range(-options.ysep,options.ysep+1):
                    for z in range(-options.zsep,options.zsep+1):
                        
                        if ( x != 0 or y != 0 or z != 0 ): # Only make a new key if the total separation isn't zero
                            # elemblock(c,x,y,z,p)
                            elemblock(c,f,f,x,y,z,1)



# FOR A BIG LOOP OVER VARIABLES
# for c in range(0,4): # loop over the cc_mass
#     # Loop over gamma matrix insertions ... for Raza
#     for f in [1, 2, 4, 8, 13, 14, 7]: # gamma1 loop
#         for g in [1, 2, 4, 8, 13, 14, 7]: #gamma2 loop
#             for p in range(-options.maxP,options.maxP+1):
