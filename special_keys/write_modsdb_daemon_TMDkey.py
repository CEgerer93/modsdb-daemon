...
parser.add_option("-a","--exttype", type="string", default="hxy0",
		  help='Set the type of data to be generated to name directories -- i.e. hxy0, hx0z (default hxy0)')
...
Dir=baseDir+"/"+beta+"/"+ensem+"/"+momInStr+"/pdf_m"+lm+"_db/"+str(src)
# Set the output name for the sdb
sdbname="/data/cpegerer/SDB_pion/"+beta+"/"+ensem+"/"+momOutStr+"/"+str(src)
...
# gamma=["0 0","1 1","2 2","4 4","1 13","2 14","14 14","13 13","11 11","15 15"] #b6p3 32x64 m0p2390
# gamma=["0 0","1 1","2 2","4 4","1 13","2 14","14 14","13 13","11 11","15 15","8 8","1 2"] #b6p3 32x64 m0p2350
...
if options.pzi==0 or options.pzi==-1 or options.pzi==1:
    ...
    # Times="5 6 7 8 9 10 11 12 13" # for 32x64 b6p3 lattices
    # numT=9 # for 32x64 b6p3 lattices
...
def elemblock2pt(cfg):
    ...
    xml.writeTag("Cfg",cfg)
    xml.writeTag("Name","pion_PDF_2pt.mod")
    xml.writeTag("Npt","2mod")
    xml.closeTag()
    # Set the location and naming of resulting sdb file
    xml.openTag("sdb")
    xml.writeTag("name",sdbname+"/pion_2pt/pion_2pt.sdb"+str(cfg))
    xml.closeTag()

def elemblock4pt(cfg,ccm,g):
    # Specify current-source information
    xml.openTag("Jsrc_sep")
    xml.writeTag("num_t",numT)
    xml.writeTag("times",Times)
    ...
    xml.writeTag("Dir",Dir)
    xml.writeTag("Cfg",cfg)
    xml.writeTag("Name","pion_PDF_4pt.mod")
    xml.writeTag("Npt","4mod")
    xml.closeTag()
    # Set the location and naming of resulting sdb file
    xml.openTag("sdb")
    xml.writeTag("name",sdbname+"/"+options.exttype+"/pion_4pt.sdb"+str(cfg))
    xml.closeTag()
...
# # Loop over 2pt keys
# for cfg in range(options.cfgmin,options.cfgmax+options.cfgstep,options.cfgstep):
#     xml.openTag("elem")
#     elemblock2pt(cfg)
#     xml.closeTag()
